...
function getProductImages() {
    ...
    return Array.from(thumbnails).map(thumb => {
        // Use getAttribute to get the full src path
        const src = thumb.getAttribute('src');

        // If the path is relative, convert it to an absolute path
        if (src && !src.startsWith('http') && !src.startsWith('/')) {
            // Assuming the images are in a consistent location relative to the current page
...
function addToCart(productId, productName, productPrice, productImage, quantityInputId) {
    ...
    function normalizeImagePath(imageSrc) {
        ...
        let cleanPath = imageSrc.replace(/^\.\.\/|^\.\//, '');
    ...
    const product = {
        ...
        image: normalizeImagePath(productImage || document.getElementById('mainImage').src),
        quantity: quantity
    };
...
function updateCart() {
    ...
    cart.forEach((item, index) => {
        // Additional check to ensure image path is valid
        ...
        cartItems.innerHTML += `
            <div class="card mb-2">
                <div class="card-body">
                    <div class="d-flex">
                        <img src="${imageUrl}" alt="${item.name}" style="width: 50px; height: 50px; object-fit: cover;" onerror="this.onerror=null; this.src='/path/to/placeholder/image.jpg';">
